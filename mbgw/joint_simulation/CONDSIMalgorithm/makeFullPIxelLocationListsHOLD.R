...
makeFullPIxelLocationLists<-function(){
 ...
 ## define sequence of pixel row and month positions
  ...
  # if we are thinning the footprint, thin the yseq vector accordingly
    if(class(THIN)!="NULL"){
      yseq<-yseq[seq(1,length(yseq),by=THIN)]
    }
    
    if(MonthDepth==0) tseq<-NULL
    if(MonthDepth!=0) tseq<--1:-MonthDepth
  ...
  # define lists of positions for all pixels: previous months up to MonthDepth
    if(MonthDepth!=0){
      xseq<--ColDepth:ColDepth  #taking ColDepth columns to both left and right of prediction column, plus the one directly underneath
  ...
  ## return pixel location list
     Obj<-list("xp"=xp,"yp"=yp,"tp"=tp,"xd"=xd,"yd"=yd,"td"=td,"yseq"=yseq)
     return(Obj)
}
#############################################################################
