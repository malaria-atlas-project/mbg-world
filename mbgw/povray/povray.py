...
class File:
  def __init__(self,fnam="out.pov",*items):
    ...
    self.__indent = 0
    [self.include(item) for item in items]
...
class Item:
  def __init__(self,name,args=[],opts=[],**kwargs):
    ...
    for i in range(len(opts)):
      if type(opts[i]) == tuple or type(opts[i]) == list:
        opts[i] = Vector(opts[i])
    self.args = args
    self.opts = opts
    self.kwargs=kwargs
  ...
  def __str__(self):
      # print self.__class__, self.args+self.opts, ", ".join([str(arg) for arg in self.args + self.opts])
      out = self.name+'{'
      out += ", ".join([str(arg) for arg in self.args])
      out += ' '.join([str(opt) for opt in self.opts])
      kwarg_strs = []
      for key, val in self.kwargs.items():
          if type(val)==tuple or type(val)==list:
              val = Vector(*val)
          kwarg_strs.append("%s %s"% (key, val))
      out += ", ".join(kwarg_strs)
      out += '}'
      return out
  def write(self, file):
...
class LightSource(Item):
  def __init__(self,*opts,**kwargs):
    Item.__init__(self,"light_source",(),opts,**kwargs)
    # else:
